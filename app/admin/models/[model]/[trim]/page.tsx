...
export default function ModelTrimDetailPage() {
  const params = useParams();
  const model = decodeURIComponent(params.model as string);
  const trim = decodeURIComponent(params.trim as string);
  ...
  const fetchListings = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/listings?model=${encodeURIComponent(model)}&trim=${encodeURIComponent(trim)}&limit=100`
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <Link href="/admin/models" className="text-blue-600 hover:text-blue-800 text-sm mb-1 inline-block">
            ← Back to Models
          </Link>
          <br />
          <Link
            href={`/admin/models/${encodeURIComponent(model)}`}
            className="text-blue-600 hover:text-blue-800 text-sm mb-2 inline-block"
          >
            ← Back to {model}
          </Link>
          <h1 className="text-3xl font-bold text-gray-900">{model} {trim}</h1>
